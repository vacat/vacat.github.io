...
categories: ["技术"]
featured: true
summary: "效率提升不解决问题，只暴露下一层瓶颈——Harness→内存→流程，瓶颈迁移是不可逆的深层结构。这篇文章提出了一个可以操作的框架：怎么识别当前瓶颈、怎么预判下一层、怎么区分真瓶颈和假瓶颈。"
---

有一个数字经常被引用：95% 的 AI 项目失败了。据说从 GPT-3 到 Claude 4.7，纹丝不动。*[B: MIT Project NANDA 2025，多份报告交叉验证。"AI 项目失败"的定义边界模糊——ROI 为负？弃用？未达预期？不同研究口径不一。]*
...
AHE 论文做过一个实验。不改模型参数。只优化七个 harness 组件——工具定义、中间件、记忆、上下文压缩、验证策略、调度逻辑、错误恢复。十轮迭代。*[A: "Agentic Harness Engineering", arxiv:2604.25850, 2025]*
...
Reiner Pope 从第一性原理推导过 LLM 推理的时间公式：推理时间 = max(T_compute, T_memory)。小 batch 时内存带宽是短板，大 batch 时计算才是瓶颈。*[A: Reiner Pope, "Efficiently Scaling Transformer Inference", arxiv:2211.05102, 2022]*
...
以上两个例证有实验数据支撑。接下来这个是我的归纳推断，证据强度不同。

AI 对工程师的效率提升，行业估算在 55-80% 之间。*[C: 多份行业调查综合区间，来源分散、口径不一，宜视为量级参考]* 对市场部？没人敢报数字。
...
流程之后是什么？一个推测：当 AI 能执行任何流程，最稀缺的会变成「知道该执行什么流程」的人。*[D: 推测，无实证]*
...
**这条规律是否可证伪？** 给它一个真实的证伪条件：如果你在一个系统上连续三轮改进后，每一次都精准预判了下一层瓶颈并且全部成功处理，但系统的整体产出没有显著提升——那说明「瓶颈迁移」这个框架在你面对的系统上失效了。瓶颈可能不存在，可能是个假循环，也可能你一直在修的不是瓶颈。三轮。这是我能给出的最诚实的证伪标准。
