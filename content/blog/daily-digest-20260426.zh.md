...
date: 2026-04-26T06:18:00+08:00
tags: [文章摘要, 日报, gpt-5-5, openai]
categories: ["技术日报"]
series: []
featured: true
...
> honker是一个将Postgres的NOTIFY/LISTEN发布/订阅语义移植到SQLite的项目,实现为"rust" SQLite扩展并提供多语言绑定。项目设计允许用熟悉的队列API编写异步工作进程,例如Python中可调用`db.queue("emails")`创建队列并用async for消费,甚至支持多进程共享队列的原子操作。
